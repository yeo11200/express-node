...
const verifyRefreshToken = (req: Request, res: Response, next: NextFunction) => {
  if (!req.headers.authorization) {
    ...
    try {
      ...
      res.locals.assetsToken = JWT.sign({ sub: 'access', data: data }, secretKey, { expiresIn : '30m' });
    }
    catch (e) {
      // 토큰이 만료가 될 경우
      if (e === 'TokenExpiredError') {
        return res.json(fail(ERROR_CODE.REFRESHTOKENEXPIRED, ERROR_MSG.REFRESHTOKENEXPIRED));
      }
      
      // 토큰이 유효하지않을 경우
      return res.json(fail(ERROR_CODE.REFRESHTOKENINVALID, ERROR_MSG.REFRESHTOKENINVALID));
    }
  }
...
const verifyToken = (req: Request, res: Response, next: NextFunction) => {
  if (!req.headers.authorization) {
    ...
    try {
      ...
      // 토큰이 만료가 될 경우
      if (e === 'TokenExpiredError') {
        return res.json(fail(ERROR_CODE.ACCESSTOKENEXPIRED, ERROR_MSG.ACCESSTOKENEXPIRED));
      }
      
      // 토큰이 유효하지않을 경우
      return res.json(fail(ERROR_CODE.ACCESSTOKENINVALID, ERROR_MSG.ACCESSTOKENINVALID));
    }
  }
...
const signToken = (data: any) => {
  ...
  // sign : JWT를 등록할 때 사용
  res.assetsToken = JWT.sign({ sub: 'access', data: data }, secretKey, { expiresIn : '30m' });
  res.refreshToken = JWT.sign({ sub: 'refresh', data: data }, secretKey, { expiresIn : '24h' });
